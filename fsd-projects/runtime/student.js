...
function generateLevel() {
  ...
  for (var i = 0; i < currentLevel.gameObjects.length; i++) {
    ...
    console.log(currentObject.type, currentObject.kind, currentObject.x);
  }
}
...
function handleHallebotGenericCollisions() {
  ...
  for (let i = 0; i < gameObjects.length; i++) {
    ...
    // Only bugs, spikes, and healthUps can alter health
    if (
      gameObject.type !== "enemy" &&
      gameObject.type !== "obstacle" &&
      gameObject.type !== "powerup"
    ) {
      continue;
    }

    if (isGenericCollision(gameObject)) {
...
function triggerLevelTransition() {
  ...
  // switch to the next level and clear previous objects
